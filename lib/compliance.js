...
export function profileChecklist(customer) {
  const items = [
    { key: "identity", label: "ID details and copies", satisfied: Boolean(customer?.id_type && customer?.id_number && customer?.id_front_url && customer?.id_back_url) },
    { key: "address", label: "Address, state and LGA", satisfied: Boolean(customer?.address && customer?.state && customer?.lga) },
    { key: "contact", label: "Email and phone", satisfied: Boolean(customer?.email && customer?.phone_number) },
    { key: "dob", label: "Date of birth and gender", satisfied: Boolean(customer?.date_of_birth && customer?.gender) },
    { key: "nok", label: "Next of kin", satisfied: Boolean(customer?.nok_name && customer?.nok_phone_number && customer?.nok_relationship && customer?.nok_gender && customer?.nok_address) },
