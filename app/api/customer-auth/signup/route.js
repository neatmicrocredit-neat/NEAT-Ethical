...
export async function POST(request) {
  try {
    ...
    const { data: customer, error } = await supabase.from("customers").insert({
      ...
      auth_user_id: authData.user.id, account_number: accountNumber(), source: "customer_portal",
    }).select("id, uuid, first_name, last_name, email, account_number").single();
    if (error) {
      await supabase.auth.admin.deleteUser(authData.user.id);
      return Response.json({ error: error.message }, { status: 500 });
    }
    try { await sendPortalSignupEmail(customer); } catch (emailError) { console.error("Signup email error:", emailError); }
